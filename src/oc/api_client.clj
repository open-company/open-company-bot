...
(defn parse-json [handler]
  (fn [request]
    (let [res (handler request)]
      (d/chain res
               #(update % :body (comp cheshire/parse-string bs/to-string))))))

(defn patch-company! [slug data]
  (http/patch (str +api-host+ "/companies/" slug) {:middleware parse-json
...
(-> (patch-company! "hotel-procrastination" (cheshire/generate-string {:description "Yada Yada"}))
    (d/chain prn)
    (d/catch #(prn (bs/to-string (:body (ex-data %))))))
